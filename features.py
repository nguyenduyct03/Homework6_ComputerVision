...
class PCA(object):
	...
	def fit(self, X, method='svd'):
		...
		if (method=='svd'):
			self.U_pca, __ = self._svd(X_centered)
		if (method=='eigen'):
			self.U_pca, __ = self._eigen_decomp(X_centered)
		# END YOUR CODE
	...
	def transform(self, X, n_components):
		...
		temp = X - self.mean
		X_proj = np.dot(temp,self.U_pca[:,0:n_components])
		# END YOUR CODE
	...
	def reconstruct(self, X_proj):
		...
		temp = np.dot(X_proj,self.U_pca[:,0:n_components].T)
		X = temp + self.mean
		# END YOUR CODE
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE
            X_i = X[np.flatnonzero(y == i)]
            mean = np.mean(X_i, axis=0)
            X_i_dt = X_i - mean
            S_i = np.dot(X_i_dt.T, X_i-mean)
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            X_i = X[np.flatnonzero(y == i)]
            mean = mu[i]
            S_i = X_i.shape[0]*np.dot((X_i-mean).T, X_i-mean)
            # END YOUR CODE
